...
The system SHALL support Jira Cloud REST API with appropriate version handling.

#### Implementation Notes (discovered during testing)

**API Version Strategy:**
- Search: Use `/rest/api/3/search/jql` (v2 deprecated with 410 Gone)
- All other endpoints: Use `/rest/api/2/` for compatibility
- v3 search requires explicit `fields` parameter (doesn't return fields by default)

**Description/Comment Format:**
- v2 API uses plain text for description and comment body
- v3 API uses Atlassian Document Format (ADF)
- Implementation uses v2 endpoints with plain text for simplicity

**Epic Linking (parent field):**
- Jira Cloud uses `parent` field for epic hierarchy
- Format: `{ parent: { key: "EPIC-123" } }` (NOT `{ parent: "EPIC-123" }`)
- Legacy instances may use `customfield_10014` or similar

**Sub-task Parent:**
- Sub-tasks use same `parent` field: `{ parent: { key: "PARENT-123" } }`
- Sub-tasks can only be created in same project as parent
